...
num_location<-length(unique(protests)) 
...
count_in_location<-function(num_area){
  num_in_state<-sum(str_detect(locations,num_area))
  result<-paste ("there were", num_in_state, "protests in", num_area)
  return(result)
}
...
in_2020<-c(dates[dates >="2020-01-01"&dates <="2020-12-31"]) 
# Create a vector of the dates that are in 2019. `in_2019`
in_2019<-c(dates[dates>="2019-01-01"&dates<="2019-12-31"])
...
count_on_date<- function(num_time){
  num_in_state<-sum(str_detect(dates,num_time))
result<-paste ("there were", num_in_state, "protest in", num_time)
return(result)
} 
...
in_2020<-c(dates[dates >="2020-07-01"&dates <="2020-07-31"]) 
in_2019<-c(dates[dates>="2019-07-01"&dates<="2019-07-31"])
change_july_protest<-length(in_2020)-length(in_2019) 
...
high_level_purpose<-str_trim(str_remove_all(purpose,"\\(.....*"))  
